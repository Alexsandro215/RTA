...
def apply_strategy(data: pd.DataFrame) -> pd.DataFrame:
    """
    Bull Trend Rider 4H Max V2.

    Experimental variant of Bull Trend Rider 4H Max.

    Compared with Max:
    - Earlier breakout entry: 60 candles instead of 72.
    - Slightly stricter heat filter: RSI 14 <= 72.
    - Same main trend filter: EMA 50 above EMA 200.
    - Same defensive exit: close below EMA 140 or EMA 50 loses EMA 200.

    In the local 2025+ BTC/USDT 4h sample it improved total return, kept the
    same validation return, and accepted a small drawdown increase.
    """
    timeframe_minutes = _infer_timeframe_minutes(data)
    if not 230 <= timeframe_minutes <= 250:
        return _no_trade(data, "Bull Trend Rider 4H Max V2 - 4h Only")

    result = _with_indicators(data)
    breakout_line = result["high"].rolling(60).max().shift(1)
    exit_line = result["low"].rolling(120).min().shift(1)
    ...
    result["strategy"] = "Bull Trend Rider 4H Max V2"
    return result
