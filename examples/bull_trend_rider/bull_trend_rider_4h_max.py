...
def apply_strategy(data: pd.DataFrame) -> pd.DataFrame:
    """
    Bull Trend Rider 4H Max.

    More aggressive 4h improvement over Bull Trend Rider 4H Plus.

    Tuned around the 2024+ BTC/USDT 4h TestBack window:
    - Trend filter: EMA 50 above EMA 200.
    - Entry: close breaks previous 72-candle high.
    - Entry heat filter: RSI 14 <= 75.
    - Exit: close below previous 160-candle low, close below EMA 140,
      or EMA 50 loses EMA 200.
    ...
    if not 230 <= timeframe_minutes <= 250:
        return _no_trade(data, "Bull Trend Rider 4H Max - 4h Only")
    ...
    result["strategy"] = "Bull Trend Rider 4H Max"
    return result
