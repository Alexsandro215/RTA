...
def apply_strategy(data: pd.DataFrame) -> pd.DataFrame:
    """
    Bull Trend Rider Guarded.

    Custom strategy for the TestBack uploader. It keeps the original
    Bull Trend Rider behavior on 15m/30m/1h, then uses a faster defensive
    exit on higher timeframes where the original 240-period Donchian exit
    tends to give too much profit back.
    ...
    if timeframe_minutes >= 7 * 24 * 60:
        return _no_trade(data, "Bull Trend Rider Guarded - Weekly Skip")

    if timeframe_minutes >= 3 * 24 * 60:
        return _apply_guarded_rider(
            data=data,
            breakout_period=48,
            exit_period=160,
            exit_ema_period=120,
            trailing_stop_pct=0.80,
            label="Bull Trend Rider Guarded 3d",
        )
    ...
    if timeframe_minutes >= 24 * 60:
        return _apply_guarded_rider(
            ...
            label="Bull Trend Rider Guarded 1d",
        )

    if timeframe_minutes >= 4 * 60:
        return _apply_guarded_rider(
            data=data,
            breakout_period=48,
            exit_period=160,
            exit_ema_period=200,
            trailing_stop_pct=0.0,
            label="Bull Trend Rider Guarded 4h/12h",
        )

    return _apply_original_rider(data)


def _apply_original_rider(
    data: pd.DataFrame,
    fast_period: int = 80,
    slow_period: int = 300,
    ...
    result = _with_emas(data, [fast_period, slow_period])
    fast_column = f"ema_{fast_period}"
    slow_column = f"ema_{slow_period}"
    breakout = result["high"].rolling(breakout_period).max().shift(1)
    exit_line = result["low"].rolling(exit_period).min().shift(1)

    buy_condition = (
        (result["close"] > breakout)
        & (result[fast_column] > result[slow_column])
    )
    sell_condition = result["close"] < exit_line

    result["signal"] = _build_position_aware_signals(buy_condition, sell_condition)
    result["strategy"] = "Bull Trend Rider Guarded - Original Intraday"
    return result


def _apply_guarded_rider(
    data: pd.DataFrame,
    breakout_period: int,
    exit_period: int,
    exit_ema_period: int,
    ...
    if trailing_stop_pct > 0:
        ...
        sell_condition = sell_condition | (result["close"] < rolling_high * trailing_stop_pct)
